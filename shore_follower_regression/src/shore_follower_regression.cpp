...
class ShoreFollowerRegression: public rclcpp::Node {
    protected:
    ...
    protected: // ROS Callbacks
        ...
        void image_callback(const sensor_msgs::msg::Image::ConstSharedPtr & img_msg) {
            ...
            bool save_it = true;
            float label = last_command_.angular.z;
            int label_index = 1;
            if (label > 0.01) label_index = 0;
            else if (label < -0.01) label_index = 2;
            save_it = (type_counter_[label_index] < (unsigned)max_image_per_type_);
            if (save_it) {
                ...
                fprintf(fp,"%04ld/%04ld.png %f\n",image_counter_/1000,image_counter_%1000,label);
                fclose(fp);
                type_counter_[label_index]++;
                image_counter_++;
            }
    ...
    public:
        ShoreFollowerRegression() : rclcpp::Node("shore_follower_regression"){
            learning_ = true;
            ...
            image_sub_ = image_transport::create_subscription(this,"~/image",
                    std::bind(&ShoreFollowerRegression::image_callback,this,std::placeholders::_1),
                    transport, rmw_qos_profile_sensor_data);
            joy_sub_ = this->create_subscription<sensor_msgs::msg::Joy>("~/joy",1,std::bind(&ShoreFollowerRegression::joy_callback,this,std::placeholders::_1));
            twist_sub_ = this->create_subscription<geometry_msgs::msg::Twist>("~/twist",1,std::bind(&ShoreFollowerRegression::twist_callback,this,std::placeholders::_1));
...
int main(int argc, char * argv[]) 
{
    ...
    rclcpp::spin(std::make_shared<ShoreFollowerRegression>());
    rclcpp::shutdown();
    return 0;
}
